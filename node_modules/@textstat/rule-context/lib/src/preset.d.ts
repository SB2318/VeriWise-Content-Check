import { TextlintRuleOptions } from "@textlint/kernel";
import { TextstatRuleMeta, TextstatRuleReporter } from "./report-type";
/**
 * Textstat Rule preset
 */
export declare type TextstatRulePreset = {
    rules: {
        [index: string]: {
            meta: TextstatRuleMeta;
            report: TextstatRuleReporter;
        };
    };
    rulesConfig: {
        [index: string]: TextlintRuleOptions | boolean;
    };
};
export declare type TextstatRulePresetFactory = {
    createPreset: () => TextstatRulePreset;
};
export declare function isTextstatRulePresetFactory(v: any): v is TextstatRulePresetFactory;
